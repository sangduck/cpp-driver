...
struct AsyncTests : public test_utils::SingleSessionTest {
  ...
  static std::vector<CassUuid> insert_async(CassSession* session,
    ...
    for (size_t i = 0; i < num_concurrent_requests; ++i) {
      ...
      std::string insert_query = str(boost::format("INSERT INTO %s (id, num, str) VALUES(%s, %s, 'row%s')") % table_name % test_utils::Value<CassUuid>::to_string(id) % i % i);
      test_utils::CassStatementPtr statement(cass_statement_new(cass_string_init2(insert_query.data(), insert_query.size()), 0));
