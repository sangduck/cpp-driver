...
BOOST_AUTO_TEST_CASE(simple)
{
  ...
  if (!boost::starts_with(conf.cassandara_version(), "1.")) {
    ...
    BOOST_TEST_MESSAGE("Unsupported Test for Cassandra v" << conf.cassandara_version() << ": Skipping serial_consistency/simple");
    BOOST_REQUIRE(true);
  }
}
...
BOOST_AUTO_TEST_CASE(invalid)
{
  ...
  if (!boost::starts_with(conf.cassandara_version(), "1.")) {
    ...
    boost::unit_test::unit_test_log_t::instance().set_threshold_level(boost::unit_test::log_messages);
    BOOST_TEST_MESSAGE("Unsupported Test for Cassandra v" << conf.cassandara_version() << ": Skipping serial_consistency/invalid");
    BOOST_REQUIRE(true);
  }
}
