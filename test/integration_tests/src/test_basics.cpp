...
struct BasicTests : public test_utils::SingleSessionTest {
  ...
  void insert_single_value(CassValueType type, T value) {
    ...
    prepared = test_utils::prepare(session, select_query);
    test_utils::CassStatementPtr select_statement(cass_prepared_bind(prepared.get()));
    ...
    insert_statement = test_utils::CassStatementPtr(cass_statement_new(cass_string_init(insert_query.c_str()), 0));
    ...
    select_statement = test_utils::CassStatementPtr(cass_statement_new(cass_string_init(select_query.c_str()), 0));
  ...
  void insert_min_max_value(CassValueType type) {
    ...
    std::string insert_query = str(boost::format("INSERT INTO %s (tweet_id, min_val, max_val) VALUES(?, ?, ?)")
                                   % table_name);
    test_utils::CassPreparedPtr prepared = test_utils::prepare(session, insert_query);
    test_utils::CassStatementPtr insert_statement(cass_prepared_bind(prepared.get()));
    BOOST_REQUIRE(cass_statement_bind_uuid(insert_statement.get(), 0, tweet_id) == CASS_OK);
...
BOOST_AUTO_TEST_CASE(min_max)
{
  ...
  // This should not be NULL since it does not reflect a minimum value
  const cass_byte_t min_byte_data[1] = { '\0' };
  CassBytes min_bytes = cass_bytes_init(min_byte_data, 1);

  {
    CassDecimal value;
    value.scale = 0;
    value.varint = min_bytes;
    insert_single_value<CassDecimal>(CASS_VALUE_TYPE_DECIMAL, value);
  }

  {
    CassString value = cass_string_init2("", 0);
    insert_single_value<CassString>(CASS_VALUE_TYPE_ASCII, value);
    insert_single_value<CassString>(CASS_VALUE_TYPE_VARCHAR, value);
  }

  {
    CassBytes value = min_bytes;
    insert_single_value<CassBytes>(CASS_VALUE_TYPE_BLOB, value);
    insert_single_value<CassBytes>(CASS_VALUE_TYPE_VARINT, value);
  }
}
...
BOOST_AUTO_TEST_CASE(counters)
{
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    test_utils::CassStatementPtr statement(cass_statement_new(cass_string_init(update_query.c_str()), 0));
...

  {
    ...
    for (size_t i = 0; i < num_rows; ++i) {
      std::string insert_query(boost::str(boost::format("INSERT INTO %s (tweet_id, t1, t2, t3) VALUES (%s, %s, %s, %s);") % test_utils::SIMPLE_TABLE
                                          % i % (i + 1) % (i + 2) % (i + 3)));
      test_utils::CassStatementPtr statement(cass_statement_new(cass_string_init(insert_query.c_str()), 0));
